...
class MermaidBoostPlugin extends Plugin {
  ...
  async exportSvgAsPng(svg) {
    try {
      ...
      beautifySvgDom(svg, this.settings, isDark);

      const nat = extractSvgNaturalSize(svg) || { x: 0, y: 0, width: 640, height: 420 };
      const scale = 3.0;
      const { themeObj, palette } = resolveThemeSpec(this.settings, isDark);
      const resolvedCardBg = this.resolveLiveCssColor(
        palette.cardBg,
        isDark ? "#191919" : "#FAF9F5"
      );
      const resolvedCardBorder = this.resolveLiveCssColor(
        palette.cardBorder,
        isDark ? "#3D3934" : "#D8D2C6"
      );
      const resolvedGridDot = this.resolveLiveCssColor(
        palette.gridDot,
      ...
      // Resolve any CSS variables (var(--...) / color-mix(...)) inside cloned SVG elements
      // so standalone Image(blob) rendering in Theme 鈶?Custom never falls back to black (#000000)
        ...
        if (el.style) {
          for (const prop of colorProps) {
            ...
            if (rawVal && (rawVal.includes("var(") || rawVal.includes("color-mix("))) {
              const resolved = this.resolveLiveCssColor(rawVal, "");
              if (resolved) {
        ...
        if (typeof el.getAttribute === "function") {
          for (const attr of ["fill", "stroke"]) {
            ...
            if (attrVal && (attrVal.includes("var(") || attrVal.includes("color-mix("))) {
              const resolved = this.resolveLiveCssColor(attrVal, "");
              if (resolved) {
      ...
      // 2. Draw Subtle Architectural Grid (Swiss Square Grid or Dot Grid)
      if (this.settings.showDotGrid !== false) {
        ctx.save();
        if (this.settings.theme === "swiss-mono") {
      ...
      if (blob && navigator.clipboard && window.ClipboardItem) {
        ...
        a.href = url;
        a.download = `mermaid-${this.settings.theme}-${Date.now()}.png`;
        a.click();
  ...
  openFullscreenLightbox(sourceSvg, diagramMeta) {
    ...
    titleEl.textContent = `${diagramMeta.label} 鈥?Fullscreen (Wheel: Zoom 路 Drag: Pan 路 Esc: Close)`;
    ...
    const updateThemeBtnText = () => {
      const activeTheme = THEMES[this.settings.theme] || THEMES["claude-anthropic"];
      themeBtn.textContent = `馃帹 ${activeTheme.name}`;
    ...
    themeBtn.addEventListener("click", async (e) => {
      ...
      beautifySvgDom(svgClone, this.settings, isDark);
      updateThemeBtnText();
      new Notice(`Theme: ${THEMES[nextKey].name}`);
    });
    ...
    makeBtn("camera", "Copy HD PNG", () => this.exportSvgAsPng(svgClone));
