...
var require_sizing = __commonJS({
  "src/sizing.js"(exports2, module2) {
    ...
    function computeSmartDiagramSize2(naturalSize, diagramMeta, settings = {}, containerWidth = 0) {
      ...
      const effectiveContainerW = Number.isFinite(containerWidth) && containerWidth > 0 ? Math.max(40, containerWidth - (containerWidth > 80 ? 32 : 16)) : maxWidth;
...
var MermaidBoostPlugin = class extends Plugin {
  ...
  decorateMermaidBlock(block, forceRestyle = false) {
    ...
    if (!svg.dataset.mbInitialized) {
      ...
      if (svg.style) {
        const origW = (typeof svg.style.getPropertyValue === "function" ? svg.style.getPropertyValue("width") : svg.style.width) || "";
        const origWPrio = typeof svg.style.getPropertyPriority === "function" ? svg.style.getPropertyPriority("width") : "";
        if (origW) {
          svg.dataset.mbOrigStyleWidth = origW;
          if (origWPrio) svg.dataset.mbOrigStyleWidthPriority = origWPrio;
        }
        const origH = (typeof svg.style.getPropertyValue === "function" ? svg.style.getPropertyValue("height") : svg.style.height) || "";
        const origHPrio = typeof svg.style.getPropertyPriority === "function" ? svg.style.getPropertyPriority("height") : "";
        if (origH) {
          svg.dataset.mbOrigStyleHeight = origH;
          if (origHPrio) svg.dataset.mbOrigStyleHeightPriority = origHPrio;
        }
        const origMW = (typeof svg.style.getPropertyValue === "function" ? svg.style.getPropertyValue("max-width") : svg.style.maxWidth || svg.style["max-width"]) || "";
        const origMWPrio = typeof svg.style.getPropertyPriority === "function" ? svg.style.getPropertyPriority("max-width") : "";
        if (origMW) {
  ...
  getContainerWidths(block) {
    ...
    const hostWidth = hostContainer && hostContainer !== document.body && hostContainer !== document.documentElement && Number.isFinite(hostContainer.clientWidth) && hostContainer.clientWidth > 0 ? hostContainer.clientWidth : 0;
    const parentWidth = block.parentElement && block.parentElement !== document.body && block.parentElement !== document.documentElement && Number.isFinite(block.parentElement.clientWidth) && block.parentElement.clientWidth > 0 ? block.parentElement.clientWidth : 0;
    const effectiveWidth = hostWidth > 0 && parentWidth > 0 ? Math.min(hostWidth, parentWidth) : hostWidth || parentWidth;
  ...
  updateDiagramSizing(block, explicitContainerWidth = null, passedEffectiveSettings = null) {
    ...
    const curW = svg.style && (typeof svg.style.getPropertyValue === "function" ? svg.style.getPropertyValue("width") : svg.style.width);
    const curH = svg.style && (typeof svg.style.getPropertyValue === "function" ? svg.style.getPropertyValue("height") : svg.style.height);
    if (svg.style && (curW !== targetW || curH !== targetH || svg.style.maxWidth !== "none")) {
  ...
  observeDiagram(block) {
    ...
    if (!this._diagramObservers) {
      this._diagramObservers = /* @__PURE__ */ new Map();
    }
    const observer = new ResizeObserver((entries) => {
      this.handleDiagramResize(block, entries);
    });
    observer.observe(block);
    if (block.parentElement && typeof document !== "undefined" && block.parentElement !== document.body && block.parentElement !== document.documentElement) {
      try {
  ...
  handleDiagramResize(block, entries) {
    ...
    const { hostWidth, parentWidth, effectiveWidth: liveContainerW } = this.getContainerWidths(block);
    const lastContainerW = block._mbLastContainerWidth;
    let hasMeaningfulResize = false;
    let callbackObservedW = null;
    if (entries && Array.isArray(entries)) {
      for (const entry of entries) {
        ...
        if (entry.target === block) {
          ...
          callbackObservedW = callbackObservedW !== null ? Math.min(callbackObservedW, calculatedObservedW) : calculatedObservedW;
          hasMeaningfulResize = true;
        } else {
          const otherW = entry.target === block.parentElement ? hostWidth : parentWidth;
  ...
  ensureExpandBar(block, fullHeight, collapsedHeight) {
    ...
    let bar = existingBars && existingBars[0] || (typeof block.querySelector === "function" ? block.querySelector(":scope > .mb-expand-bar") : null);
  ...
  ensureToolbar(block, svg, diagramMeta, displayPercent, cardPresetKey, sizingSettings = null) {
    ...
    let toolbar = existingToolbars && existingToolbars[0] || (typeof block.querySelector === "function" ? block.querySelector(":scope > .mb-toolbar") : null);
